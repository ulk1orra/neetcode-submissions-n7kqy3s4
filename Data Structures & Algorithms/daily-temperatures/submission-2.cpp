...
public:
    vector<int> dailyTemperatures(vector<int>& temperatures) 
    {
        ...
        for (int i = 0; i < temperatures.size(); i++) //проходимя по температурам 
        {
            ...
            {
                 // пара из температуры и индекса равна верхушке стека
                res[stack.top().second] = i - stack.top().second; // в результат[индкс пары] закидываем разница индексом цикла и циклом пары
